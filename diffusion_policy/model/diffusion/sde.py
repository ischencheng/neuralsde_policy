...
class NeuralSDE(torchsde.SDEIto):  # Assuming Ito SDEs are supported by default
    ...
    def __init__(
        ...
        self.prev_t = 0.0
        self.prev_dt = -1.0
    def _interpolate_cond_data(self,t):
        To=self.cond_data.shape[1]
        cond_data = t*self.cond_data[:,:To-1,:] + (1-t)*self.cond_data[:,1:To,:]
        if self.cond is not None:
            cond = t*self.cond[:,:To-1,:] + (1-t)*self.cond[:,1:To,:]
        return cond_data,cond
    ...
    def f(self, t: torch.Tensor, X: torch.Tensor):
        ...
        cond_data,cond=self._interpolate_cond_data(t)
        flow=self.flow(cond_data,t,cond)
        if self.denoiser:
            denoise=self.denoiser(cond_data)
    ...
    def g(self, t: torch.Tensor, X: torch.Tensor):
        cond_data,cond=self._interpolate_cond_data(t)
        diffusion = (torch.tanh(self.diffusion(cond_data, t, cond)) + 1.0) * 0.5 \
